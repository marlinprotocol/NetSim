...
#include "../../Networking/TransportLayer/TransportLayer.h"
...
{
	networkLayer->getTransportLayer()->setNetworkLayer(networkLayer);

	if(_routingTable == "BitcoinRoutingTable") {
		routingTable = std::make_shared<BitcoinRoutingTable>(BitcoinRoutingTable(_nodeId));
	}
	else if(_routingTable == "FlexibleRoutingTable") {
		routingTable = std::make_shared<FlexibleRoutingTable>(FlexibleRoutingTable(_nodeId));
	}
	else {
		routingTable = std::make_shared<BitcoinRoutingTable>(BitcoinRoutingTable(_nodeId));
	}
